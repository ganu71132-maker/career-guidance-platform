...
import { Plus, Pencil, Trash2, X, Save, ArrowUp, ArrowDown, ChevronDown, ChevronRight } from 'lucide-react';
...
export default function ManageRoadmaps() {
  ...
  function handleSave() {
    ...
    if (editingStep) {
      updateRoadmapStep(selectedCareer, editingStep, form);
    } else {
      addRoadmapStep(selectedCareer, form);
    }
    setShowForm(false); setEditingStep(null);
  }
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xl max-w-lg w-full">
            ...
            <div className="flex gap-3 mt-6 pt-4 border-t border-slate-100">
              ...
              <button onClick={handleSave} className="flex-1 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-medium hover:bg-emerald-500 transition-colors flex items-center justify-center gap-2">
                <Save className="h-4 w-4" /> {editingStep ? 'Save' : 'Add Step'}
              </button>
            </div>
          </div>
        </div>
