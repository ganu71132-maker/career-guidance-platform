        ...
        <div className="bg-white p-5 sm:p-6 rounded-3xl border border-slate-100 shadow-sm mb-8 sm:mb-10 relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
            <div className="space-y-2">
              ...
              {gamification && (
                <div className="mt-2 mb-4 bg-slate-50 border border-slate-100 p-3 rounded-xl max-w-sm">
                  <div className="flex justify-between items-center mb-1 text-xs font-bold text-slate-700">
                    <span>{
                      gamification.total_xp >= 800 ? 'Python Master' : 
                      gamification.total_xp >= 400 ? 'Python Professional' : 
                      gamification.total_xp >= 150 ? 'Python Explorer' : 'Python Beginner'
                    }</span>
                    <span className="text-emerald-600">{gamification.total_xp || 0} XP</span>
                  </div>
                  <div className="w-full h-2 bg-slate-200 rounded-full overflow-hidden">
                    <div className="h-full bg-emerald-500" style={{ width: `${Math.min(100, ((gamification.total_xp || 0) / (gamification.total_xp >= 800 ? 2000 : gamification.total_xp >= 400 ? 800 : gamification.total_xp >= 150 ? 400 : 150)) * 100)}%` }} />
                  </div>
                </div>
              )}

              <p className="text-xs text-slate-500 max-w-sm leading-relaxed">
                Complete at least one step in <strong>any of your saved careers</strong> or the <strong>Daily Challenge</strong> every day to keep your learning streak active!
              </p>
              <div className="pt-2 flex flex-col gap-1.5">
                <div className="flex items-baseline gap-1.5">
                  <span className="text-3xl sm:text-4xl font-extrabold text-slate-800 tracking-tight">{gamification?.current_streak || streak}</span>
